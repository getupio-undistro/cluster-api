...
func TestUpdateKubeProxyImageInfo(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fakeClient := fake.NewClientBuilder().WithObjects(objects...).WithScheme(scheme).Build()
			w := &Workload{
				Client: fakeClient,
			}
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			g := NewWithT(t)
			fakeClient := fake.NewClientBuilder().WithObjects(tt.objs...).WithScheme(scheme).Build()
			w := &Workload{
				Client: fakeClient,
			}
